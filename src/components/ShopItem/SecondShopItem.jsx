...
const SecondShopItem = (props) => {
    ...
    const EyeIconHandle = async () => {
        await axios.get(`${process.env.REACT_APP_API_ENDPOINT}/api/Products/modal/${props.product.id}`)
            .then(res => {
    ...
    const CompareHandle = async () => {
        ...
        if (!ids.includes(props.product.id) && compareCount < 3) {
            ...
            await axios.get(`${process.env.REACT_APP_API_ENDPOINT}/api/Products/compare/${props.product.id}`)
                .then(res => {
